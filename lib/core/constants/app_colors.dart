import 'package:flutter/material.dart';

class AppColors {

  static const Color sidebar = Color(0xFF1F2937);
  static const Color background = Color(0xFFF5F6FA);
  static const Color white = Colors.white;
  static const Color black = Colors.black;


  // Primary
  static const primary = Color(0xFFE8622A);
  static const darkButton = Color(0xFF070B2D);

  // Backgrounds
  static const border = Color(0xFFE8E8E8);
  static const divider = Color(0xFFF0F0F0);

  // Text Colors
  static const textPrimary = Color(0xFF1C1C1C);
  static const textDark = Color(0xFF232323);
  static const textMedium = Color(0xFF555555);
  static const textLight = Color(0xFF757575);
  static const textGrey = Color(0xFF8B8B8B);
  static const textFaded = Color(0xFF969696);

  // Status Backgrounds
  static const pendingBg = Color(0xFFFEF3C7);
  static const preparingBg = Color(0xFFDBEAFE);
  static const readyBg = Color(0xFFD1FAE5);
  static const servedBg = Color(0xFFF3E8FF);
  static const completedBg = Color(0xFFF3F4F6);

  // Status Text
  static const pendingText = Color(0xFFB45309);
  static const preparingText = Color(0xFF1D4ED8);
  static const readyText = Color(0xFF065F46);
  static const servedText = Color(0xFF6B21A8);
  static const completedText = Color(0xFF374151);

  // Others
  static const red = Colors.red;


  static const Color brown      = Color(0xFFA34613);
  static const Color brownLight = Color(0xFF8B4513);
  static const Color cream      = Color(0xFFFAF3E8);
  static const Color orange     = Color(0xFFE8621A);
  static const Color green      = Color(0xFF2E7D32);
}